...
Implement ONLY phase $ARGUMENTS from WORKPLAN.md.
...
1. Read STENOD_SSOT.md's referenced section(s) for this phase.
2. Read this phase's full entry in WORKPLAN.md — Build, Do NOT, Done
   when, Verify.
3. Re-read CLAUDE.md's "Non-negotiable constraints" and "Locked
...
between STENOD_SSOT.md and WORKPLAN.md: stop and ask me. Do not guess
or improvise a fix.
...
- Update this phase's row in WORKPLAN.md's Master Status Tracker to
  ...
  requires a separate verification pass (see CLAUDE.md and
  `/verify-phase`).
