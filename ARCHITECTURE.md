...
| Hook payloads never carry the user's own prompt text — only tool activity and the assistant's final message, which doesn't reliably restate the user's stated rationale | Resolved by scoped fallback: read transcript_path only for rejection-reasoning text, never as primary capture, wrapped behind a dedicated adapter — same isolation pattern already used for Cursor's parser — so if the format breaks, only the adapter breaks |