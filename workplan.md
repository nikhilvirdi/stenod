...
| 7.2 | `stenod start` / `stenod stop` | 6.3, 7.1 | Built (unverified) — regression guard: reverted from Verified because Phase 7.5 touched `daemon/lifecycle.ts` directly (startDaemon() now async, also starts a Phase 2.3 IPC server; DaemonHandle gained an `ipcServer` field; stopDaemon() closes it too). Existing fs-only behavior confirmed unaffected — every pre-existing Phase 7.2 test still passes unmodified. Re-verify alongside 7.5. |
| 7.3 | `stenod status` | 7.2 | Verified |
| 7.4 | Crash recovery validation | 7.2 | Blocked (Requires Unix host) |
| 7.5 | Wire terminal capture + IPC auth into the daemon | 7.2, 2.3, 5.3 | Built (unverified) — design proposal confirmed with the user before implementation (client owns the PTY via `stenod attach`, since the daemon has no TTY and node-pty cannot attach to an already-open shell; daemon reuses only `writeTerminalNode()`, not `createTerminalCapture()` as a whole). `startDaemon()` now async, starts a Phase 2.3 IPC server wired to a new daemon-side handler (`daemon/terminal-bridge.ts`) via an additive, backward-compatible `onMessage` hook added to `workspace/ipc.ts` (auth handshake itself unchanged — confirmed with the user before touching this Verified file). New client `stenod attach` (`cli/attach.ts`) reuses `TerminalWrapper`/`TerminalBatcher` unmodified. Zero lines changed in `capture/terminal.ts`, `capture/batcher.ts`, `capture/terminal-state.ts`, `network/ca.ts`, `network/proxy.ts`, `network/provider-capture.ts` (confirmed via `git diff`). **Known, explicitly-accepted limitation:** Phase 5.4's live stderr-heuristic crash detection does not fire for `stenod attach` sessions — the daemon only receives the final accumulated result at shell-exit, not live batches; only exit-code-driven `TERMINAL_SUCCESS`/`TERMINAL_ERROR` is guaranteed for bridged sessions. Also documented in `SECURITY.md`. Full local suite (Windows): 61 files, 442 passed, 3 skipped, 0 failed; `tsc --noEmit` and `lint` clean; `src/cli/e2e.test.ts` (Phase 10.7) individually reconfirmed passing. **Honest caveat:** the new real-PTY-dependent tests (`cli/attach.test.ts`'s 3 tests, one integration test in `daemon/lifecycle.test.ts`) are gated `if (isWindows) return`, matching this codebase's existing precedent (`capture/terminal-state.test.ts`) — on this Windows dev machine they only exercised their early-return path, not the real node-pty/PTY-bridge logic. A genuine attempt was made to validate for real via Docker (matching the precedent set by the original Phase 12.1 fix); Docker Desktop's Linux engine started, but two separate image pulls (`node:20`, `node:20-slim`) both stalled indefinitely at "download complete, never extracts" over ~15 minutes — the Docker environment itself appears non-functional for this in the current session, not a transient slowdown. Real Linux execution of these specific tests remains unconfirmed pending CI or a working Docker/Linux environment — flagged explicitly rather than claimed. `stenod attach` command-line UX itself (raw-mode stdin handling in `program.ts`) has not been manually exercised in a real terminal either. Phase 7.2 reverted to Built (unverified) per the regression-guard rule (see its own row). Needs a real Unix/Mac verification pass before either phase can be marked Verified. |
