...
| 12.1 | Local CA generation + trust store install | 10.7 | Built (unverified) — cert generation fully verified on this host (cross-checked via node:crypto and node-forge). Trust-store install/verify command *construction* is unit-tested for linux/darwin (exact argv asserted). **Linux real execution: CLOSED** — `trust-store.test.ts` now shells out to genuine NSS `certutil` against an isolated scratch DB (never a real trust store); gated on `process.platform === 'linux'` so it's a no-op skip on non-Linux hosts. Confirmed passing for real via a throwaway `node:22-bookworm-slim` Docker container with `libnss3-tools` installed (19/19 tests green, including both new real-execution tests) — genuine execution proof, not just command-construction assertions. Will also execute for real on CI (`ci.yml`, `ubuntu-latest`, same `libnss3-tools` package preinstalled). **macOS real execution: still OPEN** — `ci.yml` runs `ubuntu-latest` only, by deliberate design, with no macOS runner at any phase, so this cannot be closed by any automated system here; only manual execution on a real Mac can confirm it, and per explicit scope decision that verification (and any macOS-specific test logic) is deferred to Phase 12.5, not this phase. Phase 12.1 must not be marked Verified until that manual macOS check is done. |
